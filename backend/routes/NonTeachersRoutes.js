...
route.get("/", async (req, res) => {
  const data = await NonTeacherModel.find({ role: role.NonTeacher });
  res.json(data);
});
...
route.get("/:id", async (req, res) => {
  ...
  await NonTeacherModel.findOne({ userID: req.params.id })
    .then((user) => {
      if (user) {
        return res.json({ user });
      } else {
        return res.json({ error: "Staff does not exists" });
      }
    })
    .catch((err) => {
...
route.post("/create", async (req, res) => {
  ...
  const number = await NonTeacherModel.countDocuments({
  ...
  bcrypt.hash(staffID, 10, (err, hash) => {
    ...
    NonTeacherModel.create(userData)
      .then((user) => {
